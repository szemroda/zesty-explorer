...
Audit date: 2026-09-11. Scope: the local browser application and its offline tests.
...
| Errors do not expose credentials                     | Pass   | Typed errors are created from status and safe static text, never response bodies or request headers. Tests inspect serialized authentication/decoding failures for the supplied token.                                                |
